...
export const install = env.ready(async function() {
    ...
    let newNav = <nav className="bettercf-navbar"/>;
    ...
    let other = <div className="bettercf-navbar-item">
                    <a href="#">Other</a>
                </div>;
    let ddOther = <div className="bettercf-dropdown" />;

    // Iterate over all nav items and include them the new navbar
    for (let item of oldNav.children) {
        let link = item.children[0]; // <a> tag

        let newItem = <div className="bettercf-navbar-item">{link}</div>

        // Add dropdown menu, if needed
        const href = link.getAttribute('href');
        if (keys[href]) {
            let dropdown = <div className="bettercf-dropdown"/>
    ...
    // Change Codeforces logo to BetterCF logo
    let logo = dom.$('#header img');
    if (logo && /codeforces-logo/.test(logo.getAttribute('src'))) {
        logo.setAttribute('src', 'https://github.com/anomalyco/BetterCF/raw/main/assets/bettercf-logo.png');
    }
});
