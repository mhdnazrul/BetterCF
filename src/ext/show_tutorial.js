...
function showModal() {
    dom.$('.bettercf-tutorial').classList.remove('bettercf-hidden');
}

function closeModal() {
    dom.$('.bettercf-tutorial').classList.add('bettercf-hidden');
}
...
function createModalNodes() {
    let modalInner = <div className="bettercf-modal-inner">loading...</div>
    let modal =
        <div className="bettercf-modal bettercf-tutorial bettercf-hidden">
            <div className="bettercf-modal-background" onClick={closeModal}/>
            {modalInner}
        </div>;
...
export const install = env.ready(function() {
    ...
    let btn = <a className="bettercf-tutorial-btn" style="cursor: pointer;"> Tutorial </a>;
    dom.on(btn, 'click', () => {
        loadModal();
        showModal();
    });
